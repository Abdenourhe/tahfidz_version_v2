...
/*
  Change l'URL ci-dessous par ton animation Lottie préférée.
  Sources gratuites :
  • https://lottiefiles.com/free-animations/education
  • https://lottiefiles.com/free-animations/school
...
export function LottieHero() {
  return (
    <div className="relative w-full max-w-lg mx-auto">
      ...
      <motion.div animate={{ ...float, transition: { ...float.transition, delay: 1 } }} className="absolute bottom-12 -right-8 z-20">
      ...
      <motion.div animate={{ ...floatSlow, transition: { ...floatSlow.transition, delay: 1.5 } }} className="absolute -bottom-2 left-8 z-20">
      ...
      <motion.div animate={{ ...float, transition: { ...float.transition, delay: 2 } }} className="absolute top-1/2 -right-12 z-20">
      ...
      <div className="absolute -top-6 -left-6 w-24 h-24 opacity-20 z-0">
        <svg viewBox="0 0 100 100">
          {Array.from({ length: 25 }).map((_, i) => (
            <circle key={i} cx={(i % 5) * 20 + 10} cy={Math.floor(i / 5) * 20 + 10} r="2" fill="#1D9E75" />
          ))}
        </svg>
      </div>
    </div>
