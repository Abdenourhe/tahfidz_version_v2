"use client"
// src/app/admin/parents/[id]/edit/ParentEditForm.tsx
...
export function ParentEditForm({ parentId, initialData }: { parentId: string; initialData: InitialData }) {
  const { locale } = useLanguage()
  const router = useRouter()
  const t = useT("parents_2")
  const tNew = useT("parents_new")
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(true)
      setTimeout(() => router.push(`/admin/parents/${parentId}`), 1500)
    } catch (e) {
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <form onSubmit={onSubmit} className="space-y-5">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-6 space-y-4">
          <h2 className="font-semibold text-gray-800 dark:text-gray-100">{tNew("personalInfo")}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              >
                <option value="">{tNew("male") === "Masculin" ? "—" : tNew("choose") || "—"}</option>
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="isActive" className="text-sm text-gray-700 dark:text-gray-300">{t("active")}</label>
          </div>
        </div>
