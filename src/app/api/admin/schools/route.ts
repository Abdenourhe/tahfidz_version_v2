...
export async function DELETE(req: NextRequest) {
  ...
  // Supprimer une école complète (tables orphelines manuelles, puis cascade Prisma)
  if (type === "school" && id) {
    const school = await prisma.school.findUnique({ where: { id } })
    if (!school) return NextResponse.json({ error: "École introuvable" }, { status: 404 })

    await prisma.$transaction(async (tx) => {
      // Tables sans relation Prisma vers School (schoolId sans @relation)
      await tx.$executeRaw`DELETE FROM "broadcasts" WHERE "schoolId" = ${id}`
      await tx.$executeRaw`DELETE FROM "school_update_requests" WHERE "schoolId" = ${id}`
      await tx.$executeRaw`DELETE FROM "uploaded_files" WHERE "schoolId" = ${id}`

      // Le reste est géré automatiquement par les relations onDelete: Cascade de Prisma
